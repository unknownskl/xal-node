...
<Callout title="Updates expected in the future" type="warn">
  Updates are planned in the future to improve the user experience while using the library.
</Callout>
...
this._msal.doDeviceCodeAuth().then((deviceCodeDetails:any) => {
    ...
    this._msal.doPollForDeviceCodeAuth(deviceCodeDetails.device_code).then((tokens:any) => {
            ...
            console.log('User token', tokens) // Save this token for later use to persist logins.
...
## Exchange user token for a XSTS token

The XSTS token is the users session token. Use this token for the current user session.

```js
this._msal.doXstsAuthentication().then((xstsToken) => {
    console.log('xstsToken', xstsToken)
}).catch((error:any) => {
        console.log('Failed to request XSTS token', error)
})
```
